...
export function MessageInbox({ userType = 'employer' }: MessageInboxProps) {
    ...
    // --------------------------------------------------
    // Helper to fetch inbox from API
    // --------------------------------------------------
    ...
    useEffect(() => {
        ...
        console.log('[Inbox] joined user room:', profile.id)
    }, [socket, isConnected, profile?.id])
    ...
    useEffect(() => {
        ...
        const handleMessageReceived = (data: any) => {
            console.log('📥 Inbox: message_received via socket:', data)
            if (!data || !data.threadId || !data.message) {
                console.warn('⚠️ Inbox: message_received payload missing threadId or message, skipping')
                return
            }
            ...
            setInboxData(prev => {
                ...
                const shouldMarkUnread = msg.recipientId === currentUserId

                let updatedThread: Message | null = null

                const updatedMessages = prev.messages.map(thread => {
                    if (thread.threadId === data.threadId) {
                        const updatedThreadEntry: Message = {
                            ...thread,
                            content:
                                msg.content.length > 100
                                    ? msg.content.substring(0, 100) + '...'
                                    : msg.content,
                            isRead: shouldMarkUnread ? false : thread.isRead,
                        ...
                        updatedThread = updatedThreadEntry
                        return updatedThreadEntry
                    }
                    return thread
                })

                if (!updatedThread) {
                    console.warn('⚠️ Inbox: message_received for unknown thread. Fetching inbox.')
                    fetchInbox({ showSpinner: false })
                    return prev
                }

                const otherThreads = updatedMessages.filter(t => t.threadId !== data.threadId)

                return {
                    ...prev,
                    unreadCount: shouldMarkUnread ? prev.unreadCount + 1 : prev.unreadCount,
                    messages: [updatedThread, ...otherThreads],
                }
            })
        }

        const handleNewThread = (data: { thread: any; message: any }) => {
            console.log('🧵 Inbox: new_thread via socket:', data)
            ...
            setInboxData(prev => {
                ...
                const sidebarEntry: Message = {
                    ...
                    isRead: data.message.recipientId === currentUserId ? false : true,
                    ...
                    sender: {
                        ...
                        profilePictureUrl: data.message.sender.profilePictureUrl,
                        employer: data.message.sender.employer
                            ? { companyName: data.message.sender.employer.companyName }
                            : undefined,
                    },
                    otherParticipant: (() => {
                        const details = data.thread.participantDetails || []
                        const other = details.find((p: any) => p.id !== currentUserId) || details[0]
                        ...
                        return {
                            ...
                            presignedProfilePictureUrl: other.presignedProfilePictureUrl,
                            employer: other.employer ? { companyName: other.employer.companyName } : undefined,
                        }
                    })(),
                    application: data.message.application
                        ? {
                            id: data.message.application.id,
                            job: {
                                id: data.message.application.job.id,
                                title: data.message.application.job.title,
                                companyName:
                                    data.message.application.job.employer?.companyName ??
                                    data.message.application.job.companyName ??
                                    '',
                            },
                        }
                        : undefined,
                ...
                const alreadyExists = prev.messages.some(t => t.threadId === data.thread.id)
                const shouldIncrementUnread = data.message.recipientId === currentUserId
                ...
                return {
                    ...
                    messages: [sidebarEntry, ...prev.messages],
                }
            })
        }

        const handleNewMessage = (data: { threadId: string; message: any }) => {
            console.log('📥 Inbox: new_message via socket:', data)

            setInboxData(prev => {
                if (!prev) return prev
                if (!data || !data.threadId || !data.message) return prev

                const { message: msg, threadId } = data
                const existingThread = prev.messages.find(t => t.threadId === threadId)
                if (!existingThread) {
                    console.warn('⚠️ Inbox: got new_message for unknown thread. Fetching inbox.')
                    fetchInbox({ showSpinner: false })
                    return prev
                }
                ...
                const updatedThread: Message = {
                    ...existingThread,
                    content:
                        msg.content.length > 100
                            ? msg.content.substring(0, 100) + '...'
                            : msg.content,
                    isRead: shouldMarkUnread ? false : existingThread.isRead,
                    createdAt: msg.createdAt,
                    sender: {
                        ...existingThread.sender,
                        ...msg.sender,
                    },
                }

                const otherThreads = prev.messages.filter(t => t.threadId !== threadId)
        ...
        const handleUserOnline = (data: { userId: string }) => {
            console.log('👤 Inbox: user_online', data.userId)
            setOnlineUsers(prev => {
        ...
        const handleUserOffline = (data: { userId: string }) => {
            console.log('👤 Inbox: user_offline', data.userId)
            setOnlineUsers(prev => {
                const next = new Set(prev)
    ...
    }, [socket, isConnected, currentUserId, fetchInbox])
