...
import { getInvitationTokenByTeamMemberId } from '@/lib/team-invitations'
...
export async function GET(request: NextRequest) {
    try {
        ...
        // Get invitation token for current team member
        const teamMemberToken = await getInvitationTokenByTeamMemberId(teamMember.id);

        // Get invitation tokens for pending invitations
        const pendingInvitationTokens = await Promise.all(
            pendingInvitations.map(async (inv) => ({
                id: inv.id,
                token: await getInvitationTokenByTeamMemberId(inv.id)
            }))
        );
        const pendingTokenMap = Object.fromEntries(
            pendingInvitationTokens.map(({ id, token }) => [id, token])
        );
        ...
        return NextResponse.json({
            ...
            teamMember: {
                ...
                invitationToken: teamMemberToken || null,
                employer: {
            ...
            pendingInvitations: pendingInvitations.map(inv => ({
                ...
                invitationToken: pendingTokenMap[inv.id] || null,
                employer: {
